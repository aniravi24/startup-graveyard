...
const url = (process.env.DEV_ENV === 'dev') ? process.env.MONGODB_URI : process.env.PROD_MONGODB_URI
...
app.post('/api/recaptcha', cors(corsOptions), (req, res) => {
  axios.post(`https://www.google.com/recaptcha/api/siteverify?secret=${process.env.RECAPTCHA_SECRETKEY}&response=${req.body.response}`, {
  })
  .then((response) => {
